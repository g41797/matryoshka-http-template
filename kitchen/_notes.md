...
---
  ...
  Phase 2: Commit and Push the Submodule Reference in the Parent Repository
   ...
   1. Check status in the parent repository:
       * You will now see that deps/odin-http is listed as modified. This means the parent repository detects that the submodule is pointing to
         the new commit ID you just pushed.
   1     # Run from: root of main project
   2     git status
   3     # Expected output will include 'modified: deps/odin-http (new commits)'


   2. Stage the submodule reference change:
   1     # Run from: root of main project
   2     git add deps/odin-http
...
---
  ...
  This is for the actual code modifications you made inside deps/odin-http.


   1. Make Changes: Edit the files you need to within the deps/odin-http/ directory.
   ...
   3. Switch Git Context to Submodule:
       ...
       * Click this dropdown. You should see an entry for your submodule, likely named something like odin-http (deps/odin-http). Select this
         entry.
  ...
  Now, your main project needs to record that it's using this new, updated version of your odin-http submodule.
   ...
   1. Switch Git Context Back to Main Repository:
       ...
       * Click the dropdown menu at the top (which now shows odin-http (deps/odin-http)).
  ...
  hash) of the submodule has changed.
   3. Stage the Submodule Reference:
       * Hover over the deps/odin-http entry in the "Changes" list and click the + icon to stage it.
